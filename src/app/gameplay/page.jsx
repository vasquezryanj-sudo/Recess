...
export default function GameplayPage() {
  ...
  useEffect(() => {
    setOrderedPlayers(prev => [...prev].sort((a, b) => (game.scores[b.id] || 0) - (game.scores[a.id] || 0)));
  }, [game.scores]);

  const flashScore = (playerId, direction) => {
    setScoreFlash(prev => ({ ...prev, [playerId]: direction }));
    setTimeout(() => setScoreFlash(prev => { const n = {...prev}; delete n[playerId]; return n; }), 400);
  };
  ...
  return (
    <div className="screen" style={{ background: 'var(--cream)', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <div style={{ background: 'var(--navy)', padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button onClick={() => setShowGameOver(true)} style={{ background: 'var(--red)', color: 'white', border: '2px solid rgba(255,255,255,0.3)', borderRadius: '6px', padding: '6px 10px', fontSize: '0.7rem', fontFamily: 'Fredoka One, cursive', letterSpacing: '0.5px', cursor: 'pointer', opacity: 0.7 }}>
          END
        </button>
      </div>
      ...
      <div style={{ flex: 1, padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: '10px', overflowY: 'auto' }}>
        {orderedPlayers.map((player, index) => {
          ...
          return (
            <div key={player.id} className="card-retro" style={{ padding: '14px 16px', display: 'flex', alignItems: 'center', gap: '12px', transition: 'all 0.4s cubic-bezier(0.34, 1.56, 0.64, 1)', background: index === 0 ? 'var(--gold)' : 'var(--paper)' }}>
              <div style={{ fontFamily: 'Abril Fatface, serif', fontSize: '1.2rem', color: 'var(--navy)', opacity: index === 0 ? 1 : 0.4, width: '24px', textAlign: 'center' }}>{index + 1}</div>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: player.animal.color, border: '3px solid var(--navy)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.4rem', flexShrink: 0 }}>{player.animal.emoji}</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: 'Fredoka One, cursive', fontSize: '1.1rem', color: 'var(--navy)' }}>{player.name}</div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                <button
                  ...
                  style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'var(--red)', border: '2px solid var(--navy)', color: 'white', fontSize: '1.3rem', fontFamily: 'Fredoka One, cursive', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', userSelect: 'none', lineHeight: 1 }}>−</button>
                <div onClick={() => setShowInput({ playerId: player.id, sign: 1 })}
                  style={{ width: '64px', textAlign: 'center', fontFamily: 'Abril Fatface, serif', fontSize: '1.6rem', color: score < 0 ? 'var(--red)' : 'var(--navy)', background: flash === 'up' ? 'rgba(45, 106, 79, 0.2)' : flash === 'down' ? 'rgba(200, 48, 58, 0.2)' : 'transparent', borderRadius: '6px', transition: 'background 0.3s', cursor: 'pointer' }}>
                  {score}
                </div>
                <button
                  onTouchStart={() => startLongPress(player.id, 1)} onTouchEnd={() => endLongPress(player.id)}
                  onMouseDown={() => startLongPress(player.id, 1)} onMouseUp={() => endLongPress(player.id)} onMouseLeave={() => endLongPress(player.id)}
                  onClick={() => handleQuickPress(player.id, 1)}
                  style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'var(--green)', border: '2px solid var(--navy)', color: 'white', fontSize: '1.3rem', fontFamily: 'Fredoka One, cursive', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', userSelect: 'none', lineHeight: 1 }}>+</button>
              </div>
            </div>
          );
        })}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(27,42,74,0.7)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 100, backdropFilter: 'blur(4px)' }}
          ...
          <div className="card-retro" style={{ width: '100%', maxWidth: '430px', padding: '24px 20px', borderRadius: '20px 20px 0 0', background: 'var(--cream)' }}>
            <div style={{ fontFamily: 'Fredoka One, cursive', fontSize: '1rem', color: 'var(--navy)', marginBottom: '16px', letterSpacing: '1px', textTransform: 'uppercase', opacity: 0.6 }}>
              {showInput.sign > 0 ? '➕ Add points' : '➖ Subtract points'} for {orderedPlayers.find(p => p.id === showInput.playerId)?.name}
            </div>
